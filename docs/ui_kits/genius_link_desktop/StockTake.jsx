...
function StockTake({ onCancel, onCreate }) {
  const { Static, SelectField, WorkflowStrip } = window._invShared;
  const [items, setItems] = React.useState([
    { sku: 'STL-44021', name: 'Structural Steel I-Beam',  expected: '142',  counted: '140',  unit: 'PCS', location: 'A-12', notes: '2 units damaged', cost: 450 },
    { sku: 'CMT-90112', name: 'Portland Cement Type I',   expected: '1820', counted: '1820', unit: 'BAG', location: 'B-04', notes: '', cost: 24.5 },
    { sku: 'AGG-21044', name: 'Coarse Aggregate 20mm',    expected: '48',   counted: '46',   unit: 'TON', location: 'OUT-1',notes: 'Spillage 2t', cost: 125 },
    { sku: 'PLY-30022', name: 'Plywood Sheet 18mm',       expected: '312',  counted: '',     unit: 'SHT', location: 'C-08', notes: '', cost: 92 },
    { sku: 'PNT-55310', name: 'Epoxy Floor Coating',      expected: '88',   counted: '',     unit: 'L',   location: 'D-03', notes: '', cost: 38 },
    { sku: 'RBR-71203', name: 'Reinforcement Bar #6',     expected: '0',    counted: '',     unit: 'PCS', location: 'A-15', notes: '', cost: 78 },
  ]);
  ...
  const netVarValue = items
    .filter(i => String(i.counted).trim() !== '')
    .reduce((s, i) => s + ((parseFloat(i.counted) || 0) - (parseFloat(i.expected) || 0)) * (i.cost || 0), 0);
  const fmtVal = (n) => (n < 0 ? '-' : (n > 0 ? '+' : '')) + Math.abs(n).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 });
  ...
  return (
    <Page breadcrumb={['Commercial', 'Inventory', 'Stock Take']} title="Stock Take Session"
      ...
      <WorkflowStrip steps={['Open', 'Counting', 'Review', 'Posted']} current={pct === 100 ? 2 : 1} accent="#4A7CFF" />
      <Card padding={20}>
      ...
      <Card>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 16 }}>
          ...
          <STile label="Variance Value" value={`${fmtVal(netVarValue)}`} sub="SAR impact" color={netVarValue < 0 ? '#EF4444' : (netVarValue > 0 ? '#F97316' : '#1DB88A')} />
        </div>
      </Card>
