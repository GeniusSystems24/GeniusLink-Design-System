...
/* =========================================================
   ...
   ========================================================= */
// "g"-prefixed two-key sequences (Linear/GitHub style) jump to a screen.
...
function NavigationSidebar({ screen, onNavigate, theme, setTheme, mode, collapsed, mobileOpen, onCloseDrawer, dir = 'ltr' }) {
  const isMobile = mode === 'mobile';
...
function NavShell({ screen, setScreen, theme, setTheme, viewportWidth, dir = 'ltr', children }) {
  ...
  // Global shortcuts: "/" or ⌘/Ctrl-K opens search; "g"+key jumps to a screen;
  ...
  React.useEffect(() => {
    ...
    const onKey = (e) => {
      ...
      if (e.key === 'Escape') { clearPending(); if (isMobile) setMobileOpen(false); return; }
      const k = (e.key || '').toLowerCase();
      if (pending) {
        const hit = SHORTCUTS.find((s) => s.combo === pending + k);
        clearPending();
        if (hit) { e.preventDefault(); setSearchOpen(false); handleNavRef.current(hit.id); }
        return;
      }
      if (k === 'g' && SHORTCUTS.some((s) => s.combo[0] === 'g')) {
        pending = 'g';
    ...
    <div dir={dir} style={{ position: 'relative', height: '100%', display: 'flex', flexDirection: 'column', overflow: 'hidden', background: 'var(--gl-bg)' }}>
      ...
      <div style={{ position: 'relative', flex: 1, minHeight: 0, display: 'flex', overflow: 'hidden' }}>
        <NavigationSidebar screen={screen} onNavigate={handleNav} theme={theme} setTheme={setTheme}
          mode={mode} collapsed={collapsed} mobileOpen={mobileOpen} onCloseDrawer={() => setMobileOpen(false)} dir={dir} />
